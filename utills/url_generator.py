...
class ImageUrlGenerator:
    ...
    def generate_url(self):
        ...
        part = self.nmId // 1000
        self.url = f"https:{get_host(vol)}/vol{vol}/part{part}/{self.nmId}/images/{self.size}/{self.number}.{self.format}"
        return self.url

    def change_photo_number(self, photoNumber):
        vol = self.nmId // 100000
        part = self.nmId // 1000
        self.url = f"https:{get_host(vol)}/vol{vol}/part{part}/{self.nmId}/images/{self.size}/{photoNumber}.{self.format}"
        return self.url
